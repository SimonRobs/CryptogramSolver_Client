...
export class SolverService {
  ...
  constructor(private socket: Socket) {
    ...
    this.socket.fromEvent<string>(SocketMessages.ANSWER).subscribe((data: string) => this.parseResult(data))
  }
  ...
  private parseResult(result: string): void {
    ...
    wordsSplit.pop(); // Remove 'Enter' character

    const foundWords = [...this._result.value.words];
    if (foundWords.length === 0) {
      wordsSplit.forEach((word: string) => foundWords.push({ values: [word] }))
    } else {
